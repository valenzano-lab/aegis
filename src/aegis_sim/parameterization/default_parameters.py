...
DEFAULT_PARAMETERS = {
    ...
    "CARRYING_CAPACITY_EGGS": Parameter(
        ...
        domain="other",
        default=500,
        info="Maximum number of eggs that the environment can sustain; -1 means no limit",
        info_extended="Once the number of eggs exceeds the carrying capacity of eggs, newly laid eggs replace previously laid eggs. Set to -1 to disable the limit entirely. The limit applies even when hatching/incubation is instant.",
        dtype=int,
        drange="{-1, [1, inf)}",
        inrange=lambda x: x == -1 or x >= 1,
        serverrange=lambda x: x == -1 or x <= 10000,
        serverrange_info="{-1, [1,10000]}",
        evalrange=[-1, 1000000],
