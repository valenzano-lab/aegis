...
DEFAULT_PARAMETERS = {
    ...
    "LATTICE_MODE": Parameter(
        key="LATTICE_MODE",
        name="",
        domain="ecology",
        default=False,
        info="Enable hexagonal lattice spatial model (one individual per cell, toroidal wraparound, lattice constrains mating + offspring placement + migration)",
        info_extended="When True, individuals occupy positions on a toroidal hexagonal lattice. Mating becomes spatial: a female searches expanding rings outward for a fertile male. Offspring are placed in a random adjacent empty cell (birth fails if no adjacent empty cell). Individuals migrate to adjacent empty cells (and rarely, to random distant empty cells). When False (default), AEGIS behavior is unchanged.",
        dtype=bool,
        drange="{True, False}",
        inrange=lambda x: isinstance(x, bool),
        evalrange=[True, False],
    ),
    "LATTICE_TARGET_DENSITY": Parameter(
        key="LATTICE_TARGET_DENSITY",
        name="",
        domain="ecology",
        default=0.3,
        info="Target occupancy fraction at carrying capacity (only used when LATTICE_MODE=True)",
        info_extended="Sets the lattice size at sim init: n_cells = expected_carrying_capacity / LATTICE_TARGET_DENSITY. Higher values = denser populations (mate-finding easier, less Allee effect, closer to well-mixed AEGIS). Lower values = sparser populations (stronger spatial structure, more isolation-by-distance, stronger Allee effect). Recommended baseline: 0.3.",
        dtype=float,
        drange="(0, 1]",
        inrange=lambda x: 0 < x <= 1,
        evalrange=[0.1, 0.3, 0.5, 0.9],
    ),
    "MIGRATION_RATE": Parameter(
        ...
        domain="ecology",
        default=0.1,
        info="Per-individual per-step probability of local migration to an adjacent empty cell",
        info_extended="Only used when LATTICE_MODE=True. The dominant viscosity knob. Low values = strong isolation-by-distance. High values = effectively well-mixed.",
        dtype=float,
        drange="[0, 1]",
        inrange=lambda x: 0 <= x <= 1,
        evalrange=[0.0, 0.01, 0.1, 0.5],
    ),
    "MIGRATION_LONG_RATE": Parameter(
        key="MIGRATION_LONG_RATE",
        name="",
        domain="ecology",
        default=0.005,
        info="Per-individual per-step probability of long-distance dispersal to a random empty cell anywhere on the lattice",
        info_extended="Only used when LATTICE_MODE=True. Provides a slow genetic-mixing channel even at high viscosity. Set to 0 for pure isolation-by-distance. Real species typically have nonzero long-distance dispersal (storm-driven, seed-mediated, etc.).",
        dtype=float,
        drange="[0, 1]",
        inrange=lambda x: 0 <= x <= 1,
        evalrange=[0.0, 0.001, 0.005, 0.05],
    ),
    "MATING_MAX_SEARCH_RADIUS": Parameter(
        key="MATING_MAX_SEARCH_RADIUS",
        name="",
        domain="ecology",
        default=3,
        info="How many hex rings outward a female searches for a fertile male",
        info_extended="Only used when LATTICE_MODE=True and REPRODUCTION_MODE=sexual. Ring 1 = the 6 adjacent cells, ring 2 = the next 12 cells, etc. The female picks a fertile male at random from the closest non-empty ring. If no fertile male within MATING_MAX_SEARCH_RADIUS rings, no reproduction this step. Mitigates Allee effects at low density; higher values = weaker spatial mating constraint.",
        dtype=int,
        drange="[1, inf)",
        inrange=lambda x: x >= 1,
        evalrange=[1, 3, 5, 10],
