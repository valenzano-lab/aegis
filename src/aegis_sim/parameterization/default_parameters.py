...
DEFAULT_PARAMETERS = {
    ...
    "G_selectable_via_surv_lo": Parameter(
        key="G_selectable_via_surv_lo",
        name="",
        domain="genetics",
        default=0,
        info="Minimum value of the selectable_via_surv trait",
        dtype=float,
        drange="",
    ),
    "G_selectable_via_surv_hi": Parameter(
        key="G_selectable_via_surv_hi",
        name="",
        domain="genetics",
        default=1,
        info="Maximum value of the selectable_via_surv trait",
        dtype=float,
        drange="",
    ),
    #
    ...
    "G_surv_custom_weights": Parameter(
        key="G_surv_custom_weights",
        name="",
        domain="composite genetic architecture",
        default=None,
        info="Bit weights used when G_surv_interpreter is custom_weighted",
        dtype=list,
        drange="A list with one non-negative value per bit",
    ),
    ...
    "G_repr_custom_weights": Parameter(
        key="G_repr_custom_weights",
        name="",
        domain="composite genetic architecture",
        default=None,
        info="Bit weights used when G_repr_interpreter is custom_weighted",
        dtype=list,
        drange="A list with one non-negative value per bit",
    ),
    ...
    "G_neut_custom_weights": Parameter(
        key="G_neut_custom_weights",
        name="",
        domain="composite genetic architecture",
        default=None,
        info="Bit weights used when G_neut_interpreter is custom_weighted",
        dtype=list,
        drange="A list with one non-negative value per bit",
    ),
    ...
    "G_muta_custom_weights": Parameter(
        key="G_muta_custom_weights",
        name="",
        domain="composite genetic architecture",
        default=None,
        info="Bit weights used when G_muta_interpreter is custom_weighted",
        dtype=list,
        drange="A list with one non-negative value per bit",
    ),
    ...
    "G_grow_custom_weights": Parameter(
        key="G_grow_custom_weights",
        name="",
        domain="composite genetic architecture",
        default=None,
        info="Bit weights used when G_grow_interpreter is custom_weighted",
        dtype=list,
        drange="A list with one non-negative value per bit",
    ),
    "G_selectable_via_surv_evolvable": Parameter(
        key="G_selectable_via_surv_evolvable",
        name="",
        domain="composite genetic architecture",
        default=False,
        info="Is selectable_via_surv an evolvable trait?",
        info_extended="When True, selectable_via_surv gets its own heritable genome loci and its phenotype "
        "is used (via G_selectable_via_surv_mode) to add a selection-derived correction to the surv "
        "phenotype. Only supported under the composite genetic architecture.",
        dtype=bool,
        drange="",
    ),
    "G_selectable_via_surv_agespecific": Parameter(
        key="G_selectable_via_surv_agespecific",
        name="",
        domain="composite genetic architecture",
        default=False,
        info="Is selectable_via_surv age-specific?",
        info_extended="If True, one locus (and one selection-derived surv correction) per age, matching "
        "surv's age resolution. If False, a single value per individual, broadcast to all ages of surv.",
        dtype=bool,
        drange="",
    ),
    "G_selectable_via_surv_interpreter": Parameter(
        key="G_selectable_via_surv_interpreter",
        name="",
        domain="composite genetic architecture",
        default="uniform",
        info="",
    ...
    "G_selectable_via_surv_initgeno": Parameter(
        key="G_selectable_via_surv_initgeno",
        name="",
        domain="composite genetic architecture",
        default=0.5,
        info="Initial selectable_via_surv genotype frequency",
        dtype=float,
        drange="",
    ),
    "G_selectable_via_surv_custom_weights": Parameter(
        key="G_selectable_via_surv_custom_weights",
        name="",
        domain="composite genetic architecture",
        default=None,
        info="Bit weights used when G_selectable_via_surv_interpreter is custom_weighted",
        dtype=list,
        drange="A list with one non-negative value per bit",
    ),
    #
    #
    # SELECTABLE_VIA_SURV SELECTION FUNCTION
    "G_selectable_via_surv_mode": Parameter(
        key="G_selectable_via_surv_mode",
        name="",
        domain="selectable_via_surv selection",
        default="directional_selection",
        info="How the selectable_via_surv phenotype is translated into a correction added to surv",
        dtype=str,
        drange="{directional_selection, stabilizing_selection}",
        inrange=lambda x: x in ("directional_selection", "stabilizing_selection"),
    ),
    "G_selectable_via_surv_directional_sel_cutoff_value": Parameter(
        key="G_selectable_via_surv_directional_sel_cutoff_value",
        name="",
        domain="selectable_via_surv selection",
        default=0.5,
        info="directional_selection: trait value at or above which the constant benefit is applied",
        dtype=float,
        drange="",
    ),
    "G_selectable_via_surv_directional_sel_constant_benefit": Parameter(
        key="G_selectable_via_surv_directional_sel_constant_benefit",
        name="",
        domain="selectable_via_surv selection",
        default=0,
        info="directional_selection: constant added to surv when trait value >= cutoff",
        dtype=float,
        drange="",
    ),
    "G_selectable_via_surv_directional_sel_constant_penalty": Parameter(
        key="G_selectable_via_surv_directional_sel_constant_penalty",
        name="",
        domain="selectable_via_surv selection",
        default=0,
        info="directional_selection: constant added to surv when trait value < cutoff",
        info_extended="Typically negative.",
        dtype=float,
        drange="",
    ),
    "G_selectable_via_surv_stabilizing_sel_mean": Parameter(
        key="G_selectable_via_surv_stabilizing_sel_mean",
        name="",
        domain="selectable_via_surv selection",
        default=0.5,
        info="stabilizing_selection: mean of the Gaussian benefit function",
        dtype=float,
        drange="",
    ),
    "G_selectable_via_surv_stabilizing_sel_sd": Parameter(
        key="G_selectable_via_surv_stabilizing_sel_sd",
        name="",
        domain="selectable_via_surv selection",
        default=0.1,
        info="stabilizing_selection: standard deviation of the Gaussian benefit function",
        dtype=float,
        drange="(0, inf)",
        inrange=lambda x: x > 0,
    ),
    "G_selectable_via_surv_stabilizing_sel_max_benefit": Parameter(
        key="G_selectable_via_surv_stabilizing_sel_max_benefit",
        name="",
        domain="selectable_via_surv selection",
        default=0,
        info="stabilizing_selection: peak of the Gaussian (value added to surv when trait value == mean)",
        dtype=float,
        drange="",
    ),
    "G_selectable_via_surv_stabilizing_sel_const_penalty": Parameter(
        key="G_selectable_via_surv_stabilizing_sel_const_penalty",
        name="",
        domain="selectable_via_surv selection",
        default=0,
        info="stabilizing_selection: constant added to surv beyond G_selectable_via_surv_stabilizing_sel_const_penalty_beyond_sd standard deviations from the mean",
        info_extended="Typically negative.",
        dtype=float,
        drange="",
    ),
    "G_selectable_via_surv_stabilizing_sel_const_penalty_beyond_sd": Parameter(
        key="G_selectable_via_surv_stabilizing_sel_const_penalty_beyond_sd",
        name="",
        domain="selectable_via_surv selection",
        default=2,
        info="stabilizing_selection: number of standard deviations from the mean within which the Gaussian is used",
        info_extended="Beyond this many standard deviations from the mean, G_selectable_via_surv_stabilizing_sel_const_penalty is used instead of the Gaussian.",
        dtype=float,
        drange="[0, inf)",
        inrange=lambda x: x >= 0,
    ),
    ...
    "G_selectable_via_surv_initpheno": Parameter(
        key="G_selectable_via_surv_initpheno",
        name="",
        domain="modifying genetic architecture",
        default=0.0,
        info="",
